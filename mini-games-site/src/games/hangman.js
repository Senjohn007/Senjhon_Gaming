...
export function initHangman() {
  ...
  const hangmanSvg = document.getElementById("hangman-svg") || createHangmanSvg();
  const keyboardDiv = document.getElementById("hangman-keyboard") || createKeyboard();
  ...
  function createKeyboard() {
    ...
    // Create keyboard buttons for each letter
    for (let i = 0; i < 26; i++) {
      const letter = String.fromCharCode(97 + i); // a-z
      const button = document.createElement("button");
      button.textContent = letter.toUpperCase();
      button.className = "hangman-key-btn bg-slate-700 hover:bg-slate-600 text-white font-bold py-2 px-3 rounded transition-colors";
    ...
    // Insert after the input and guess button
    const inputContainer = input.parentNode;
    inputContainer.parentNode.insertBefore(keyboardContainer, inputContainer.nextSibling);
  ...
  if (!document.getElementById('hangman-styles')) {
    ...
    styleElement.innerHTML = hangmanStyles;
  ...
  function endGame(result) {
    ...
    // Update info div with appropriate styling
    infoDiv.className = result;
    if (result === "win") {
  ...
  function updateDisplay() {
    ...
    // Display wrong letters with styling
    wrongDiv.innerHTML = "";
    if (wrongGuesses.length > 0) {
      wrongDiv.innerHTML = `<div>Wrong letters: </div>`;
    ...
    // Update hangman figure based on wrong guesses
    const parts = [
      "hangman-head",
      "hangman-body",
      "hangman-leftarm",
  ...
  function handleGuessClick() {
    ...
    const keyBtn = document.querySelector(`.hangman-key-btn[data-letter="${letter}"]`);
    if (keyBtn) {
